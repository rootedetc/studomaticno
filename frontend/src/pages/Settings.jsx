...
export default function Settings() {
  ...
  const { theme, setTheme, language, setLanguage, notifications, toggleNotifications, clearCache } = useSettings();

  const testNotification = async () => {
    if ('Notification' in window) {
      if (Notification.permission === 'granted') {
        new Notification('studomaticno', {
            ...
            {['light', 'dark', 'system'].map((themeOption) => (
              ...
              >
                <div className="flex flex-col items-center gap-2">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${theme === themeOption ? 'bg-primary-100 dark:bg-primary-900/40' : 'bg-gray-100 dark:bg-gray-700'
                    }`}>
                    {themeOption === 'light' && (
          ...
          <div className="p-4 space-y-4">
            <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              ...
              <button
                ...
                className={`relative w-14 h-7 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 ${notifications ? 'bg-primary-600' : 'bg-gray-300 dark:bg-gray-600'
                  }`}
              >
                <span
                  className={`absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow-md transition-transform duration-200 ${notifications ? 'translate-x-7' : 'translate-x-0'
                    }`}
                />
              </button>
            </label>
