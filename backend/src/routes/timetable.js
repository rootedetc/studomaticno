...
router.post('/navigate', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (lastViewState.has(userId)) {
      ...
      const homeHtml = await edunetaService.getPage('/lib-student/raspored.aspx', requestId);
    ...
    const responseHtml = await edunetaService.postFormData('/lib-student/raspored.aspx', formData.toString(), requestId);
    ...
    log('debug', requestId, 'Cached new viewstate for user', { userId });
...
router.get('/', requireAuth, async (req, res) => {
  ...
  try {
    const html = await edunetaService.getPage('/lib-student/raspored.aspx', requestId);
    ...
    log('debug', requestId, 'Cached initial viewstate for user', { userId });
